...
#[sqlx::test]
async fn test_project_submission_and_rejection_workflow(pool: PgPool) {
    let app = TestApp::new(pool.clone()).await;
    
    let (_member, member_token) = UserFixture::create_member(&pool).await;
    let (_, top_lead_token) = UserFixture::create_toplead(&pool).await;
    
    // 1. Member submits a new project
    let create_payload = serde_json::json!({
        "title": "Bad Project",
        "description": "This project violates terms",
        "tech_stack": ["Malware"],
        "slug": "bad-project",
        "github_link": null,
        "tags": ["Bad"],
    ...
    // 2. TopLead rejects the project
    let reject_payload = serde_json::json!({
        "status": "rejected",
    });
    let approve_uri = format!("/api/projects/{}/approve", project_id);
    let res = app.post_json(&approve_uri, &reject_payload, Some(&top_lead_token)).await;
    assert_eq!(res.status(), StatusCode::OK);
    
    // 3. Project is NOT visible in the public list
    let res = app.get("/api/projects", None).await;
    let body = crate::helpers::read_body_json(res).await;
    
    let projects = body["data"].as_array().unwrap();
    let found = projects.iter().any(|p| p["id"] == project_id);
    assert!(!found, "Rejected project should NOT be visible in public list");
}
